...
def generate_schemes_set(nodes_num, branches_num, voltage_sources_num, current_sources_num, resistors_num):
    ...
    for i in range(len(circuits_topologies_paired)):
        ...
        is_unique = True  # предполагаем, что схема уникальна, пока не найдём совпадение

        # Проверяем, есть ли такая схема в уже найденных уникальных
        for existing_topology, existing_circuit in unique_schemes:
            if compare_layouts(circuit_1.layout, existing_circuit.layout) and compare_topologies(topology_1,
                                                                                                 existing_topology):
                is_unique = False
                break
    ...
    # Количество уникальных схем
    uniq_schemes_idx = len(unique_schemes)
    print(uniq_schemes_idx)
