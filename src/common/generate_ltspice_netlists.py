...
def generate_ltpsice_netlist(save_path, circuit_layout, elements_values):
    ...
    for connection_name, branches in circuit_layout.items():
        ...
        for branch in branches:
            if isinstance(branch, list):
                ...
                if 'elements' in branch.keys():

                    for index, element in enumerate(branch['elements']):
                        if index == 0:
                            if len(branch['elements']) == 1:
                        ...
                        elif index == len(branch['elements']) - 1:
                    ...
                    netlist_lines.append(
                        f"QUADRIPOLECONNECTION N{node_start:03} N{node_end:03} QUADRIPOLECONNECTION"
                    )
