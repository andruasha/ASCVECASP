...
from src.common.draw_functions import draw_resistor, draw_capacitor, draw_inductor, draw_current_source, draw_voltage_source, draw_circle
...
class CircuitVisualize:
    ...
    def visualize(self):
        ...
        # TODO: сделать проверку что между точками есть элементы
        peaks = self.get_peaks()
        draw_circle(peaks['ul'], "1")
        draw_circle(peaks['ur'], "2")
        draw_circle(peaks['dr'], "2'")
        draw_circle(peaks['dl'], "1'")
    ...
    def get_peaks(self):
        peaks = {
            'ul': {'x': float('inf'), 'y': float('-inf')},
            'ur': {'x': float('-inf'), 'y': float('-inf')},
            'dr': {'x': float('-inf'), 'y': float('inf')},
            'dl': {'x': float('inf'), 'y': float('inf')}
        }

        for branches in self.circuit.layout.values():
            for branch in branches:
                for dot in branch[0]['connection_coords']:
                    if (dot['x'] <= peaks['ul']['x']) and (dot['y'] >= peaks['ul']['y']):
                        peaks['ul']['x'] = dot['x']
                        peaks['ul']['y'] = dot['y']

                    if (dot['x'] >= peaks['ur']['x']) and (dot['y'] >= peaks['ur']['y']):
                        peaks['ur']['x'] = dot['x']
                        peaks['ur']['y'] = dot['y']

                    if (dot['x'] >= peaks['dr']['x']) and (dot['y'] <= peaks['dr']['y']):
                        peaks['dr']['x'] = dot['x']
                        peaks['dr']['y'] = dot['y']

                    if (dot['x'] <= peaks['dl']['x']) and (dot['y'] <= peaks['dl']['y']):
                        peaks['dl']['x'] = dot['x']
                        peaks['dl']['y'] = dot['y']
