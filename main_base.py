...
def generate_schemes_set(nodes_num, branches_num, voltage_sources_num, current_sources_num, resistors_num, inductors_num,
    ...
    if nodes_num == 2:
        circuits_topologies = [ElectricCircuit(branches_num=branches_num, nodes=template_two_nodes_1).create_nodes_connections() for _ in range(30)]
    elif nodes_num == 3:
        circuits_topologies = [ElectricCircuit(branches_num=branches_num, nodes=template_three_nodes_1).create_nodes_connections() for _ in range(500)]
    elif nodes_num == 4:
        circuits_topologies = [ElectricCircuit(branches_num=branches_num, nodes=template_four_nodes_1).create_nodes_connections() for _ in range(250)]
        circuits_topologies += [ElectricCircuit(branches_num=branches_num, nodes=template_four_nodes_2).create_nodes_connections() for _ in range(250)]
        random.shuffle(circuits_topologies)

    # TODO: Это работает не так как нужно для схем с 4 узлами, т.к. присутствует два типа размещения узлов
